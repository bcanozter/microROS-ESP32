...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
  ...
  if (timer != NULL) {
    ...
    if (frame) {
      ...
      camera_info.header.frame_id = micro_ros_string_utilities_set(
...
void micro_ros_task(void *arg) {
  ...
  bool success = micro_ros_utilities_create_message_memory(
      ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, CompressedImage), &image,
      conf);
  if (!success) {
  ...
  // This is dummy data
  // Ensure to calibrate camera using
  // https://github.com/ros-perception/image_pipeline/tree/jazzy/camera_calibration
  camera_info.header.frame_id = micro_ros_string_utilities_set(
      camera_info.header.frame_id, "camera_link");
  camera_info.distortion_model =
      micro_ros_string_utilities_set(camera_info.distortion_model, "plumb_bob");
...
void app_main() {
  ...
  xTaskCreate(micro_ros_task, TASK_TAG, CONFIG_MICRO_ROS_APP_STACK, NULL,
